...
import { fixedInput } from "./fixed-input.mjs";
...
// v2 abstract deltas could exceed its irrelevant measured stock defaults. v3
// supplies feasible nonnegative historical counts without changing the deltas,
// cold inputs, or predeclared preference expectations; all expanded inputs print.
for (const scenario of scenarios) scenario.rows = scenario.rows.map((entry) => entry.partition === "measured" ?
  { ...entry, stars: Math.max(entry.stars, entry.starsDelta), forks: Math.max(entry.forks, entry.forksDelta) } : entry);
...
function evaluate(rows, config) {
  ...
  const signal = (r, key) => r.partition === "cold-start" ? r[key] / Math.max(1, r.ageDays) : config.normalizeInterval ? Math.max(0, r[`${key}Delta`]) * 86400000 / (r.intervalHours * 3600000) : Math.max(0, r[`${key}Delta`]);
  ...
  const scored = rows.map((r) => {
    ...
    const recovery = Math.max(0, Math.min(1, (Math.round(elapsed * 86400000) - 7 * 86400000) / (30 * 86400000)));
    ...
    return { id: r.id, partition: r.partition, score, sortKey: Math.round(score * 1e12), base, recovery, count90, frequency, novel: elapsed >= 30, fallback, cohortMembers: peers.map((p) => p.id), attention };
  }).sort((a, b) => Number(a.partition === "cold-start") - Number(b.partition === "cold-start") || b.sortKey - a.sortKey || compareCodePoints(a.id, b.id));
...
const productAlignment = scenarios.map((scenario) => {
  const input = fixedInput(scenario.rows), actual = rankGitHub(input);
  const projected = evaluate(scenario.rows, baseline);
  const differences = [];
  if (JSON.stringify(actual.selectedNodeIds) !== JSON.stringify(projected.selected)) differences.push("selected mismatch");
  if (JSON.stringify(actual.candidates.map((r) => r.nodeId)) !== JSON.stringify(projected.order)) differences.push("total order mismatch");
  for (const candidate of actual.candidates) {
    const expected = projected.scores.find((r) => r.id === candidate.nodeId);
    for (const [product, simulation] of [["score", "score"], ["sortKey", "sortKey"], ["baseScore", "base"], ["recoveryMultiplier", "recovery"], ["frequencyMultiplier", "frequency"], ["reportCount90d", "count90"], ["novel", "novel"]])
      if (candidate[product] !== expected[simulation]) differences.push(candidate.nodeId + ":" + product + " differs");
  }
  return { scenarioId: scenario.id, input, actual, differences };
});
if (productAlignment.some((entry) => entry.differences.length)) process.exitCode = 1;
console.log(JSON.stringify({ precisionVersion: 3, productAlignment, evidence: "Owned simulation, not real satisfaction or production qualification", priorExpectationsSha256: "41ad8312dce52fe0ed5aa24771f97db29062d9d7ad6232dd10c8c18ff88db872", scenarios, trials }, null, 2));
