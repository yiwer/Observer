...
最新更新：Owner已授权按07:30/08:30启用本机每日自动邮件，配置与运行条件见[定时任务](daily-schedule.md)。下文“手动/未启用定时”描述的是此前交付阶段，手动入口仍保留，不能据此否定最新定时授权。
...
每条新闻/项目的 `significance` 必填一句，HTML 与 MD 均显示“AI 一句话解读”，说明主要影响对象/方向并保留不确定性，不冒充已验证因果。财经附11项独立行情卡片：布伦特原油期货、COMEX黄金期货、费城半导体、标普500、道琼斯工业、纳斯达克综合、韩国综合、日经225、上证综指、创业板指、恒生科技。

采集器 `daily-markets.mjs` 读取 Yahoo Finance 公开端点，无需新增 Key；这是未承诺稳定契约的公开接口，不宣称实时/SLA或交易所授权分发。报价、涨跌额/百分比、参考价、单位、报价时点、市场时区与具体期货名称来自冻结的 `acquisition.markets`。**不用5日图表的 `chartPreviousClose` 冒充昨收**；涨跌字段不全或不一致时保留明确缺口。指数单位为点，布油美元/桶，黄金美元/金衡盎司；期货不等于现货。

行情不通过新闻发布时间过滤，也不占5—7条新闻配额；各市场有不同时区/休市，实际旧交易日报价显式注明，超过7日不提供有效行情。不能把最近可得视作实时确认。模型只返回各可用行情ID的简短影响解读，价格与涨跌由程序原样格式化，缺失项保留卡片及原因，不让AI补数或臆测涨跌原因。财经即便没有合格新闻，仍可生成行情面板。每封财经邮件公示新增源、期货口径、涨跌基准和覆盖限制。

以上适用于新采集的新run；已发邮件保持不变，不回填或重发。未发旧缓存缺少新必填解读/行情时需使用新run，不把旧缓存当作已采用新规则。来源研究见 [行情源核实](../research/daily-market-sources-2026-09-08.md)。
...
只验证新行情面板而不重新搜索六栏新闻或发送邮件：`node scripts/daily-market-preview.mjs`。它对11项各请求一次并调用一次本机Codex，结果写入 Git 忽略的 `data/operator/markets-preview-<timestamp>/`；此为行情专用本地预览，不是完整六栏新日报，也不在邮件入口的run目录内。
