...
2026-09-08 按 Owner 最新请求落地的手动流程：真实来源采集、账户权限检查、本机已登录 Codex 编写，**每栏独立一封 HTML 邮件**。只收北京时间昨天00:00至本次采集冻结点内发布的信息；旧事件在此窗口新报道可以收录，窗口外旧稿和未知发布时间不收。这里不要求每栏七条，也不要求 PDF；有数据就写，来源或权限问题明确提示。
...
采集、生成输入、来源引用及发送前复用同一发布时间判定。采集开始时冻结截止点，而不是每个HTTP响应各自变化。只有整个日期落入窗口的日精度元数据可收录，不伪造小时；仅有今天日期、但不能确定早于截止点的资料须补时间或排除。热榜观察、仓库推送、普通编辑时间均不能替代发布时间；GitHub 新建仓库和新 Release 分别标明创建/发布依据。旧版“今天上榜即可”的规则已被最新请求覆盖。`render` 只重新排版尚未尝试发送的报告；一旦任一栏目有 SMTP 尝试记录就拒绝改动。
