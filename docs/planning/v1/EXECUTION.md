...
- 当前实施票：#13 GitHub重大进展一次性重新入榜，base **ec9b91c3e8575f7f3f3dc363d1d35ffb6319fce3**，专属`Observer-worktrees/v1-13` / `ticket/v1-13`，仍为未提交产品WIP。Release/GHSA及文本缓解已进入多期公开回归，出版配置独立承诺已修复原反例；Root另验配置三场、整run遗漏及五份旧Record9字节通过。动量精确参数、episode证明与有界六表持久方案已批准；176为新增能力字段契约RED，177为真实小时采集后的首次动量入榜业务RED，作者正在实现startup最小GREEN。持续高/reset/容量等动量闭环、整票固定SHA双轴review、全量及实际master验收均未完成，不能关闭或推进#14。所有切片范围与证据见[#13记录](acceptance/13-github-repromotion-events.md)，不把局部PASS当整票通过。
- 2026-09-07因Owner询问再次实时核对：GitHub #1–#12全部CLOSED、#13–#28仍OPEN，没有漏关已验收票。Owner插入的[Sandcastle＋Docker研究](../../research/sandcastle-docker-agent-sandbox-2026-09-07.md)已完成并交付，当前仅未提交研究文件；不采纳为V1依赖、不改变Runner/PRD/ADR/tickets，也没有安装/执行外部代码。当前主线仍为#13。
