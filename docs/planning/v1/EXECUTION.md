...
- 当前实施票：#8，**可版本化兴趣配置与全球覆盖底线**。原生依赖 #7 已 CLOSED，基线包含其最终集成与验收；已实际创建 fresh-context `/root/implement_v1_08`，worktree `O:/GenesisCode/Observer-worktrees/v1-08`、branch `ticket/v1-08`、fixed base **b1b6ac3ab557f0098ae5b133f20f4ca32b9182aa**，创建后 clean。已要求亲读 implement/TDD/领域和票规范，先汇报文件配置、版本快照、可解释排序与 Global Baseline 判定/首片计划，再在既有 T1 seam 逐片实现；未有产品修改或测试结论。见 [#8执行记录](acceptance/08-explicit-interest-and-global-coverage.md)。
