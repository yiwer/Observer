...
- #4 阶段报告已跑通隔离协议进程到 Gate/正文、12 类终态失败及 timeout/cancel 清理；模型通道正改为 network=none 容器内 loopback broker + stdio 控制帧，不再依赖 internal bridge 的宿主隔离假设。Linux CLI/模型协议替身的拒绝测试仍在推进。尚无冻结候选，Root 未做最终复跑/双轴验收；[阶段回写](https://github.com/yiwer/Observer/issues/4#issuecomment-5550338282)。
