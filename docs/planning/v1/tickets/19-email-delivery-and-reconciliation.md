...
2026-09-05 Owner 选定 QQ SMTP 授权码并确认发件与收件同一邮箱，详见 [最新运行输入](../OWNER-INPUTS.md)。具体地址在 ignored 本地配置，不复制至公开仓库/Issue；授权码通过秘密环境变量注入。已有一次 SMTP 预检及 Owner 收件/中文确认，该历史记录不代表 #19 产品邮件链已实测或已启用日常发送。
