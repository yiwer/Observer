...
2026-09-08 最新 Owner 决定覆盖下方早期前置描述：已确认本机已登录 Codex，并明确跳过国家审查。native 入口适配和一次有界本机真实调用可继续，不再请求接入路径或国家/地区；不要求单独 API 计费。待适配后记录实际 native 身份，不能伪填容器/地区 PASS。云主机、存储、来源和成品交付仍分别保留未测，见 [Owner 输入](../OWNER-INPUTS.md) 和 [ADR-0006](../../../adr/0006-use-native-codex-for-first-local-validation.md)。
