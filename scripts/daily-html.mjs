...
function historyUrls() {
  ...
  if (existsSync(root)) for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    if (!existsSync(join(directory, 'smtp-result.json')) || !existsSync(join(directory, 'report.json'))) continue;
    if (readJson(join(directory, 'smtp-result.json')).state !== 'accepted') continue;
...
function safeSources(ids, evidence) {
  return [...new Set(ids)].map(id => {
    ...
    return { id, title: item.title, url: url.href, source: item.source, publishedAt: item.publishedAt ?? null };
  });
}

async function generateEdition(edition, acquisition, date, directory) {
  const outputPath = join(directory, `${edition}.json`);
  if (existsSync(outputPath)) return readJson(outputPath);
  const prior = historyUrls();
  const evidence = acquisition.items.filter(item => item.edition === edition).map(item => ({ ...item,
...
资料JSON：\n${JSON.stringify(evidence)}`;
  ...
  const resultEdition = { edition, ...parsed, stories: parsed.stories.map(story => ({ ...story, sources: safeSources(story.evidenceIds, evidence) })),
...
async function main() {
  ...
  if (action === 'collect') {
    ...
    if (existsSync(join(directory, 'report.json'))) {
      ...
      if (!existsSync(join(directory, 'daily.html'))) saveNew(join(directory, 'daily.html'), rendered.html);
      if (!existsSync(join(directory, 'daily.md'))) saveNew(join(directory, 'daily.md'), rendered.text);
    ...
    if (!address) throw new Error('recipient-environment-missing');
    if (existsSync(join(directory, 'smtp-attempt.json'))) throw new Error('send-already-attempted-inspect-receipt-do-not-blindly-retry');
    const key = secret('QQ_SMTP_KEY');
    if (!key) throw new Error('qq-smtp-key-missing');
    const messageId = `<daily-${runId}@observer.invalid>`;
    ...
    const mail = await composer.sendMail({ from: { name: 'Observer 日报', address }, to: address,
      subject: `六栏日报｜${report.date} 更新版`, messageId,
      html: readFileSync(join(directory, 'daily.html'), 'utf8'), text: readFileSync(join(directory, 'daily.md'), 'utf8'),
      disableFileAccess: true, disableUrlAccess: true });
    saveNew(join(directory, 'smtp-attempt.json'), { runId, messageId, state: 'attempting', at: new Date().toISOString() });
    const transport = createQqAttachmentTransport({ enabled: true, transport: 'qq-smtp', address }, () => key);
    try {
      const result = await transport.send({ from: address, to: address, messageId, raw: mail.message });
      saveNew(join(directory, 'smtp-result.json'), { ...result, messageId, at: new Date().toISOString() });
      log({ phase: 'email-result', ...result });
    ...
    log({ runId, collected: existsSync(join(directory, 'acquisition.json')), generated: existsSync(join(directory, 'report.json')),
      mail: existsSync(join(directory, 'smtp-result.json')) ? readJson(join(directory, 'smtp-result.json')) :
        existsSync(join(directory, 'smtp-attempt.json')) ? { state: 'unknown-inspect-before-retry' } : { state: 'not-attempted' } });
  }
}
