...
export function arrangeEvents(record: GatedRecord, research: EditionResearch, history: EventRecord[] = [], legacyHistory = false, withheldHistory = new Set<string>()): EventRecord {
  ...
  for (const story of record.stories) {
    ...
    if (legacyHistory) { gaps.push({ edition: story.edition, reason: `event-selection:${story.id}:legacy-history-unclassified` }); continue; }
  ...
  for (const [id, members] of groups) {
    ...
    const unseen = developments.filter((development) => !previous.some((cluster) => cluster.developmentIds.includes(development.id)));
    ...
    eventClusters.push({ id, eventKind: labels[0]!.eventKind, memberStoryIds: members.map((story) => story.id), evidenceIds,
      ...
      occurrence: occurrence.atUtc === null && last ? historyUnavailable ? noHistoricalTime : last.occurrence : occurrence, firstDisclosure: historyUnavailable ? noHistoricalTime : last?.firstDisclosure ?? disclosure,
      ...
      developmentIds: [...new Set(displayedDevelopmentIds)], developments, supportingClaims, previousCoverage: last ? { versionId: last.primary.versionId, storyId: last.primary.storyId } : null,
  ...
  return { ...arranged, schemaVersion: 4, editorialContract: "observer-canonical-v2", coverageGaps: [...arranged.coverageGaps, ...gaps], eventClusters: retained, eventSelections };
}
