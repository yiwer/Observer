...
type EventRecord = Extract<ReportRecord, { schemaVersion: 4 | 5 | 6 }>;
export function consistentEvents(record: EventRecord, lookup: (versionId: string) => PublishedReport | undefined): boolean {
  ...
  const earlier = (id: string) => {
    ...
    return (report?.record.schemaVersion === 4 || report?.record.schemaVersion === 5 || report?.record.schemaVersion === 6) && report.version.id === id && report.record.businessDate < record.businessDate && report.version.publishedAtUtc <= record.evidenceBundle.cutoffUtc && report.record.evidenceBundle.cutoffUtc <= record.evidenceBundle.cutoffUtc ? report.record : undefined;
  };
