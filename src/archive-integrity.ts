...
export function consistentArchive(report: PublishedReport, storedId: string): boolean {
  ...
  if (record.schemaVersion === 11) return (version.schemaVersion === 10 && !record.recovery || version.schemaVersion === 11 && !!record.recovery &&
    ...
    version.provenance === (record.publicationMode ?? "test-fixture") && "editorialContract" in version && record.editorialContract === version.editorialContract && version.reportRecordSha256 === inputDigest(record) && consistentRoutedRecord(record) && routedMarkdown(record) === canonicalMarkdown;
