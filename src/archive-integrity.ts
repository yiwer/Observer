...
export function consistentArchive(report: PublishedReport, storedId: string): boolean {
  ...
  if (record.schemaVersion === 11) return version.schemaVersion === 10 && record.editorialContract === version.editorialContract && version.reportRecordSha256 === inputDigest(record) && consistentRoutedRecord(record) && routedMarkdown(record) === canonicalMarkdown;
