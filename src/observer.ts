...
function reportFromRow(row: Record<string, unknown>): PublishedReport {
  ...
  if (report.record.schemaVersion === 10) {
...
export function createObserver(options: ObserverOptions) {
  ...
  const historicalReport = (versionId: string): PublishedReport | undefined => {
    const row = database.prepare("SELECT id,payload,development_capture_sha256 FROM reports WHERE id = ?").get(versionId);
    if (!row) return undefined;
    const report = reportFromRow(row);
    if ((report.record.schemaVersion !== 4 && report.record.schemaVersion !== 5 && (report.record.schemaVersion !== 6 && report.record.schemaVersion !== 7 && (report.record.schemaVersion !== 8 && report.record.schemaVersion !== 9 && report.record.schemaVersion !== 10))) || !consistentArchive(report, versionId)) throw new ObserverError("history-integrity-failed");
    return report;
  };
  ...
  function githubHistory(businessDate: string, cutoffUtc: string): GitHubCoverageHistory {
    ...
    for (const row of database.prepare("SELECT id,payload,development_capture_sha256 FROM reports WHERE id < ? ORDER BY id").all(`${businessDate}-v1`)) {
      let old: PublishedReport;
      try {
        old = reportFromRow(row);
      ...
      if (old.record.schemaVersion !== 8 && old.record.schemaVersion !== 9 && old.record.schemaVersion !== 10) {
      ...
      const nodeIds = old.record.schemaVersion === 10 ? old.record.githubRepromotion.selectedNodeIds : old.record.schemaVersion === 8 ? old.record.github.watchItems.map((item) => item.nodeId) : old.record.githubRanking.selectedNodeIds;
  ...
  function githubEventHistory(businessDate: string, cutoffUtc: string, scope: DevelopmentHistoryReadScope): GitHubEventHistory {
    ...
    const rows = database.prepare("SELECT id,payload,development_capture_sha256 FROM reports WHERE id < ? ORDER BY id LIMIT 10001").all(`${businessDate}-v1`);
    if (rows.length > 10000) return { entries: [], unavailableVersionIds: rows.map((row) => String(row.id)) };
    for (const row of rows) {
      let old: PublishedReport;
      try { old = reportFromRow(row); if (!consistentArchive(old, String(row.id))) throw new Error("invalid-history"); }
      catch { history.unavailableVersionIds.push(String(row.id)); continue; }
      if (old.record.businessDate >= businessDate || old.version.publishedAtUtc > cutoffUtc || old.record.evidenceBundle.cutoffUtc > cutoffUtc || old.record.schemaVersion !== 10) continue;
  ...
  return {
    ...
    async produce(input: unknown, runOptions?: { signal?: AbortSignal }) {
      ...
      const parsedRequest = ProduceRequestSchema.or(SixEditionRequestSchema).or(EventEditionRequestSchema).or(InterestEditionRequestSchema).or(DomainEditionRequestSchema).or(DiscourseEditionRequestSchema).or(GitHubEditionRequestSchema).or(GitHubHeatRequestSchema).or(GitHubRepromotionRequestSchema).safeParse(input);
      ...
      if (stories.every((story) => story.schemaVersion === 2)) {
        ...
        if (research) {
          if (request.schemaVersion === 3 || request.schemaVersion === 4 || request.schemaVersion === 5 || (request.schemaVersion === 6 || (request.schemaVersion === 7 || request.schemaVersion === 8 || request.schemaVersion === 9))) {
            ...
            const history = database.prepare("SELECT id,payload,development_capture_sha256 FROM reports WHERE id < ? ORDER BY id").all(`${request.businessDate}-v1`).flatMap((row) => {
              let old: PublishedReport;
              try {
                old = reportFromRow(row);
              ...
              if (old.record.schemaVersion !== 4 && old.record.schemaVersion !== 5 && (old.record.schemaVersion !== 6 && old.record.schemaVersion !== 7 && (old.record.schemaVersion !== 8 && old.record.schemaVersion !== 9 && old.record.schemaVersion !== 10))) {
      ...
      const report = PublishedReportSchema.parse({
        version: {
          ...
          ...(record.schemaVersion === 10 || record.schemaVersion === 9 || record.schemaVersion === 8 || record.schemaVersion === 3 || record.schemaVersion === 4 || record.schemaVersion === 5 || (record.schemaVersion === 6 || record.schemaVersion === 7) ? { schemaVersion: record.schemaVersion - 1, editorialContract: record.editorialContract, reportRecordSha256: digest(JSON.stringify(record)) } : {}),
        },
      ...
      try {
        ...
        if (request.schemaVersion === 9) developmentHistoryRead((scope) => {
          if (report.record.schemaVersion !== 10) throw new ObserverError("github-publication-input-changed");
        ...
        const insertion = database.prepare("INSERT INTO reports (id,payload,development_capture_sha256) VALUES (?,?,?) ON CONFLICT(id) DO NOTHING")
          .run(report.version.id, JSON.stringify(report), capturedDevelopment);
    ...
    readReport(versionId: string, credential: string | undefined): PublishedReport {
      ...
      if ((report.record.schemaVersion === 8 || report.record.schemaVersion === 9 || report.record.schemaVersion === 10)) {
        ...
        try {
          ...
          if (report.record.schemaVersion === 9 || report.record.schemaVersion === 10) for (const entry of report.record.githubRanking.history.entries) {
            ...
            if (!old || entry.businessDate >= report.record.businessDate || entry.publishedAtUtc > report.record.evidenceBundle.cutoffUtc ||
              ...
              (old.record.schemaVersion !== 8 && old.record.schemaVersion !== 9 && old.record.schemaVersion !== 10)) throw new Error("invalid-history");
            const nodes = old.record.schemaVersion === 10 ? old.record.githubRepromotion.selectedNodeIds : old.record.schemaVersion === 8 ? old.record.github.watchItems.map((item) => item.nodeId) : old.record.githubRanking.selectedNodeIds;
      ...
      if ((report.record.schemaVersion === 4 || report.record.schemaVersion === 5 || (report.record.schemaVersion === 6 || report.record.schemaVersion === 7 || (report.record.schemaVersion === 8 || report.record.schemaVersion === 9 || report.record.schemaVersion === 10))) && !consistentEvents(report.record, historicalReport)) throw new ObserverError("canonical-integrity-failed");
      if (report.record.evidenceBundle.schemaVersion !== 1) {
        try {
          const identities = report.record.evidenceBundle.schemaVersion === 2 ? report.record.evidenceBundle.evidence : [...report.record.evidenceBundle.evidence.flatMap((evidence) => evidence.origin.kind === "collected" ? [{ sourceId: evidence.sourceId, ...evidence.origin }] : []), ...(report.record.schemaVersion === 4 || report.record.schemaVersion === 5 || (report.record.schemaVersion === 6 || report.record.schemaVersion === 7 || (report.record.schemaVersion === 8 || report.record.schemaVersion === 9 || report.record.schemaVersion === 10)) ? report.record.eventClusters.flatMap((cluster) => cluster.historyPolicies) : [])];
